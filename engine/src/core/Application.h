...
namespace ScrapGameEngine
{
    ...
    private:
        ...
        // Declaration order is load-bearing: members are initialised in the order they
        // are declared here, not the order given in the constructor's initialiser list.
        // `window` is constructed from `windowData`, so `windowData` must come first.
